...
func (g *LB_EN) BATTLE(ctx *context.Context) engine.HandlerFunc {
	...
	if len(code.ParamBytes) <= next {
		...
		if var2 == 0 {
			next = GetParam(code.ParamBytes, &var3, next)
			next = GetParam(code.ParamBytes, &exprStr, next, 0, g.ExprCharset)
			next = GetParam(code.ParamBytes, &msgStr_jp, next, 0, g.TextCharset)
...
func (g *LB_EN) TASK(ctx *context.Context) engine.HandlerFunc {
	code := ctx.Code()
	var TaskID uint16
	var TaskVar1 uint16
	var TaskVar2 uint16
	...
	if len(code.ParamBytes) <= next {
		...
		if len(code.ParamBytes) <= next {
			...
			next = GetParam(code.ParamBytes, &msgStr_jp1, next, 0, g.TextCharset)
			GetParam(code.ParamBytes, &msgStr_en1, next, 0, g.TextCharset)
			ctx.Script().SetOperateParams(ctx.CIndex, ctx.RunMode,
				TaskID,
				TaskVar1,
				TaskVar2,
			...
			next = GetParam(code.ParamBytes, &msgStr_jp1, next, 0, g.TextCharset)
			next = GetParam(code.ParamBytes, &msgStr_en1, next, 0, g.TextCharset)
			next = GetParam(code.ParamBytes, &msgStr_jp2, next, 0, g.TextCharset)
			GetParam(code.ParamBytes, &msgStr_en2, next, 0, g.TextCharset)
			ctx.Script().SetOperateParams(ctx.CIndex, ctx.RunMode,
				TaskID,
				TaskVar1,
				TaskVar2,
		...
		if len(code.ParamBytes) <= next {
			...
			next = GetParam(code.ParamBytes, &exprStr1, next, 0, g.ExprCharset)
			next = GetParam(code.ParamBytes, &exprStr2, next, 0, g.ExprCharset)
			GetParam(code.ParamBytes, &exprStr3, next, 0, g.ExprCharset)
			ctx.Script().SetOperateParams(ctx.CIndex, ctx.RunMode,
				TaskID,
				TaskVar1,
				&script.StringParam{
			...
			next = GetParam(code.ParamBytes, &TaskVar2, next)
			next = GetParam(code.ParamBytes, &exprStr1, next, 0, g.ExprCharset)
			GetParam(code.ParamBytes, &exprStr2, next, 0, g.ExprCharset)
			ctx.Script().SetOperateParams(ctx.CIndex, ctx.RunMode,
				TaskID,
				TaskVar1,
				TaskVar2,
...
func (g *LB_EN) SAYAVOICETEXT(ctx *context.Context) engine.HandlerFunc {
	code := ctx.Code()
	var voiceId uint16
	var msgStr_jp string
	var msgStr_en string
...
func (g *LB_EN) EQU(ctx *context.Context) engine.HandlerFunc {
	code := ctx.Code()
	var value uint16
	var exprStr string
...
func (g *LB_EN) EQUN(ctx *context.Context) engine.HandlerFunc {
	code := ctx.Code()
	var key uint16
	var value uint16
...
func (g *LB_EN) EQUV(ctx *context.Context) engine.HandlerFunc {
	code := ctx.Code()
	var key uint16
	var value uint16
...
func (g *LB_EN) VARSTR_SET(ctx *context.Context) engine.HandlerFunc {
	code := ctx.Code()
	var varstrId uint16
	var varstrStr string

	next := GetParam(code.ParamBytes, &varstrId)
	GetParam(code.ParamBytes, &varstrStr, next, 0, g.TextCharset)
	ctx.Script().SetOperateParams(ctx.CIndex, ctx.RunMode,
		varstrId,
		&script.StringParam{
			Data:   varstrStr,
...
func (g *LB_EN) MOVE(ctx *context.Context) engine.HandlerFunc {
	code := ctx.Code()
	var val1 uint8
	var val2 uint16
	var val3 uint16
